import os
import logging
from ..database.models import ConsultationRequest, HelpRequest
...
async def send_help_request_notification(help_request: HelpRequest) -> bool:
    """
    Send email notification to admin when a new help request is submitted.
    """
    admin_email = os.getenv("ADMIN_EMAIL")
    ...
    # Format the request date
    created_at_formatted = help_request.created_at.strftime("%d/%m/%Y %H:%M")

    # Vietnamese HTML content for admin notification
    html_content = f"""
    <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
        <h2 style="color: #2854A8;">Yêu cầu hỗ trợ mới</h2>
        <p>Bạn đã nhận được một yêu cầu hỗ trợ mới từ hệ thống VietJusticIA.</p>
        ...
        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Họ và tên:</td>
                    <td style="padding: 8px 0;">{help_request.full_name}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Email:</td>
                    <td style="padding: 8px 0;">{help_request.email}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Chủ đề:</td>
                    <td style="padding: 8px 0;">{help_request.subject}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Ngày gửi:</td>
                    <td style="padding: 8px 0;">{created_at_formatted}</td>
        ...
        <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; border-left: 4px solid #ffc107; margin: 20px 0;">
            <h3 style="color: #333; margin-top: 0;">Nội dung yêu cầu</h3>
            <p style="white-space: pre-wrap; margin: 0;">{help_request.content}</p>
        </div>

        <p style="margin-top: 30px;">Vui lòng đăng nhập vào hệ thống admin để xử lý yêu cầu này.</p>
        <p><strong>Mã yêu cầu:</strong> #{help_request.id}</p>
    ...
    subject = f"[VietJusticIA] Yêu cầu hỗ trợ mới từ {help_request.full_name}"

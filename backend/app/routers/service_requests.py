...
def get_service_request_detail(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Database error fetching service request detail: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        db.rollback()
        logger.error(f"Error fetching service request detail: {e}", exc_info=True)
        raise HTTPException(
...
def update_service_request_status(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Database error updating service request: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        db.rollback()
        logger.error(f"Error updating service request: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error"
        )
...
def delete_service_request(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Database error deleting service request: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        db.rollback()
        logger.error(f"Error deleting service request: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error"
        )
